...
\begin{comment}
\todo{That sounds super slow. How many boundary value pairs did you do?}
\end{comment}

\todo[inline]{What is \cref{heats} trying to tell us? How does it fit into the narrative? Cynthia: I think I addressed this somewhat by elaborating in the first paragraph on this page.}
...
Our method of plotting the pixel arrays for this equation is slightly different than plotting the array for the heat equation. It is essentially a stricter form of the epsilon method. An outline of the method is:

\begin{enumerate}
    \item Let the discrete reaction diffusion expression be $f(u_{i+1}, u_i, u_{i-1})$, and calculate its value.
    \item  Round that expression to the nearest bin, and round 0 to the nearest bin\footnote{We can write possible bin values $b_j$ as $b_j = mj + c$ for $j \in \mathbb{Z}$, $j \in [0,r]$ where $r$ is the resolution. However, we can expand this by making $j$ unbounded; $j \in \mathbb{Z}$ is the only requirement. An ``expanded bin value" is any value of $b_j = mj + c$}. If those rounded values are the same, then the corresponding entry in the Pixel Array is nonempty.
\end{enumerate}
