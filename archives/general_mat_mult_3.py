...
# For homogeneous diffusion equation with non-constant diffusion coefficient
# Note that theoretically, only the steady state test should change for
# different equations.
# Will be fixing general_mat_mult_2.py to reflect that
...
def steadyStateTest(orderedvars,params,dim):
    ...
    if (params[0] == 0): # if diffusion coefficient is 0, then any value is possible
        return True
...
def main():
    # Actual testing time
    rU1, dim1 = makeU(0,5,40,[-2]) # -2 is the current x coordinate
    rU2, _ = makeU(0,5,40,[-1]) # symmetric
    rU3, _ = makeU(0,5,40,[0])
    # rU4, rU5 should not be different from rU1 and rU2
    # so testing just rU1 = rU2 is enough
    rU1 = np.array(rU1)
    rU2 = np.array(rU2)
    
    # print ("This is dim1: " + str(dim1))
    # print ("This is dim2: " + str(dim2))
...
##    print (str(a.mult(b)))
    ...
    U3 = labeledTensor(rU3, ['k','l','m'], [40,40,40])
    U4 = labeledTensor(rU1, ['l','m','n'], [40,40,40])
    # if rU1 is not equal to rU2,
    # this definition of U5 will capture that
    # by ultimately producing non-symmetric solutions
