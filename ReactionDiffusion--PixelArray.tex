...
\usepackage[margin=1in]{geometry} % changes all margins to 0.5 in
\usepackage{verbatim}
\usepackage{url}
\usepackage{hyperref}
...
\affil[1]{Massachusetts Institute of Technology, Department of Electrical Engineering and Computer Science, Class of 2020}
\affil[2]{Supervisor and Principal Investigator, Massachusetts Institute of Technology, Department of Mathematics}
...
The lines in the diagram are \textit{links}, while the circles are \textit{cells} (which is referred to in \citep{Introduction_to_PA} as a \textit{pack}). The cells represent one variable value, while the links specify dependencies between these variables. There is a link from a cell to another cell if that second cell uses the first in determining its value. In other words, because $u_i$ in equation (\ref{discrete heat}) depends on $u_{i+1}$ and $u_{i-1}$, there are links from the cells representing $u_{i+1}$ and $u_{i-1}$ to the cell for $u_i$. Finally, observe that although the value in cell 0 affects the value in cell 1, the value in cell 1 does not alter the value in cell 0 because cell 0 acts as a true fixed boundary, even though the ``boundary conditions" according to Figure (\ref{WD_heat}) are the values of the exposed variables.
...
I will prove convergence for the closest numerical approximation to the worst case scenario describe by equation (\ref{max_diverge}). The closest numerical approximation to the worst case scenario sets the value of equation (\ref{discrete heat}) at the center of each subcube to be $\pm 2b$. We will assume homogeneous trivial boundaries $u_0 = u_{p+1} = 0$, and then at the end I will briefly explain how to generalize to arbitrary boundaries.
