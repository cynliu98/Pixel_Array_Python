...
\todo[inline]{The discussion of true fixed boundaries vs.\ boundaries, artificial restriction, etc.---both in the preceding paragraph and the following paragraph---is unclear. Can you clarify it? If you'd like me to, we should go over it again so I remember the situation.}

Because we expose the value of cells that are 1 away from the true boundary, we might inadvertently lose steady states\todo{Why?}. This is because a global maximum or minimum may be achieved at cells 0 or n for a partition of $n$ cells\todo{What does this have to do with losing steady states?}. Removing these boundaries, therefore, may artificially restrict the range of possible discrete steady state solution values. We resolve this issue as follows: first, a desired range is given. Secondly, we expand the range of possible solution values by a factor of $n/(n-2)$. Then, we adjust the number of bins so the size of bins $b$ remains constant. Finally, we run the program on this extended range, and then post-process the steady states to ensure that all reported solutions lie within the desired range.\todo[inline]{Huh? Maybe you can give fewer details and give the reader just enough information that she could do it on her own. Or maybe you can give more details or an example to explain it.}

To see which variables should be exposed (should be the dimensions of the product matrix) upon computing a product, draw boundaries, also called a cell, around the two cells whose corresponding matrices are being multiplied. Those links with a wire coming out of the cell are the variables that will be exposed. So for the matrices $M_1$, $M_2$ (with $M_i$ representing the value of $u_i$) in \cref{WD_oneproduct}, whose dimensions correspond to $u_0, u_1, u_2$ and $u_1, u_2, u_3$ respectively, the product's dimensions should be $u_0, u_2, u_3$.
\todo[inline]{I also don't understand the above paragraph. Why did we change the wiring diagram? Have any products been computed here? How do we know which links we want to expose?}
