...
To apply the PASS method, we need to convert the reaction-diffusion equation \eqref{Reaction-Diffusion Equation} with $u_t = 0$ to a linear equation\todo{In general, finite differences doesn't give linear equations, right?} \begin{comment} I'm not completely sure what you mean by this. Used alone, finite differences should give linear equations? As in none of the discrete u terms are multiplied together. But depending on the reaction-diffusion equation, we might get non-linear equations overall, as shown by W-J \end{comment}. This is done using finite differences, which uses secant lines to approximate derivatives. For instance,
...
As a more numerical example, consider the equation $x + y = 1$. If the range of values each variable can assume within one row or column has size 1, then the entry associated with $x \in [0,1)$, $y \in [.3,1.3)$ would be ``True." This is because there exists a solution $(x',y')$ in the given ranges to $x + y = 1$. \todo[inline]{What is the function of this paragraph? Might we consider deleting it and using the example in Fig 1 instead?}

It is useful to set up a naming convention which relates the tensor entries and the cubes they correspond to. We thus refer to an entry by the value exactly at the center of its range; e.g.\ in the above pixel would be  known as $(0.5, 0.8)$. When we wish to refer to all of an entry's possible values as opposed to just the center value, we refer to it as a \textit{subcube}. We say that the \textit{bin-size}---meaning the width of each variable---for this pixel is $(1,1)$. For simplicity, we will assume that the bin sizes of all variables are equal, for every system we discuss.
...
\noindent The corresponding tensor has three dimensions, corresponding to the variables $u_{i+1}$, $u_i$, and $u_{i-1}$ in \cref{discrete RD}. Each tensor entry corresponds to a subcube; let $(x,yz)$ denote the center point of that subcube in space. The value in that entry will be the corresponding solution-set for the equation \cref{discrete heat}. That is, the value will be the empty set $\{\}$ if either the empty set if $(x,y,z)$ is not sufficiently close to a solution, and it will be the one-element set $\{(x,y,z)$\} if $(x,y,z)$ is sufficiently close to a solution.\todo{Is this correct? Say what ``sufficiently close'' means.}
...
The lines in the diagram are \textit{links}, while the circles are \textit{cells} (which is referred to in \citep{Introduction_to_PA} as a \textit{pack}). The cells represent one variable value, while the links specify dependencies between these variables. There is a link from a cell to another cell if that second cell uses the first in determining its value. In other words, because $u_i$ in \cref{discrete heat} depends on $u_{i+1}$ and $u_{i-1}$, there are links from the cells representing $u_{i+1}$ and $u_{i-1}$ to the cell for $u_i$. Finally, observe that although the value in cell 0 affects the value in cell 1, the value in cell 1 does not alter the value in cell 0 because cell 0 acts as a true fixed boundary, even though the ``boundary conditions" according to \cref{WD_heat} are the values of the exposed variables.
