...
To test and also use the PA method, we needed to adapt it so as to produce a full solution set, rather than just existence of solutions in chosen variables. We refer to the result as the Pixel Array Solution-Set method (PASS). The ``bounding box" in this case consists of a pair of ranges for boundary values, one on each side of the one-dimensional reaction-diffusion system. The PASS method numerically finds steady states to all boundary values within the specified range. With this method, we can quickly and efficiently search for steady states of reaction-diffusion equations, which do not appear to be well-understood.

The plan of the paper is as follows. In \cref{sec:methods} we provide a brief review of the finite differences method, as well as describe our modification of the PA method. In \cref{sec:verification} we use a combination of these methods to discretize the heat equation, the local Fisher equation, and what we will call the \textit{Wilhelmsson-Jancel} equation, and calculate numerical solutions.
